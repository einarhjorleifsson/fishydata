...
tmpdir <- tempdir()
unzip("data-raw/ais/astdB/astd_classb_2014-01-01_2024-12-31.zip", exdir = tmpdir)
...
for(y in years) {
  ...
  for(m in 1:12) {
    directory |> filter(year == y, month == m) |> pull(file) |>
      ...
      duckdbfs::write_dataset("data/ais/astdB",partitioning = c("year", "month"), format = "parquet")
  }
}
