...
keep2 |>
  filter(!is.na(vid) | str_starts(mmsi, "251") | str_starts(cs, "TF") | is.na(uid)) |>
...
keep |> lh()
keep |>
  filter(is.na(.loid), .glid == "cs2") |>
  arrange(-pings) |> pull(loid) |> unique()
...
keep2 <-
  ...
  mutate(vid =
           case_when(
             .loid == "vid" & .glid == "vid" & glid %in% isl.vid ~ glid,
             .loid == "vid" & .glid == "cs" & loid %in% isl.vid & glid %in% cs.isl  ~ loid,
             .loid == "vid" & is.na(.glid) & str_starts(tolower(glid), "afma") ~ loid,
             # typo
             #loid == "2500" & glid == "THAE" ~ loid,
             #loid == "1807" & glid == "TMP_TFOP" ~ loid,
             #loid == "2654" & glid == "TMP_TFCG" ~ loid,
             .default = NA),
         cs =
           case_when(
             .loid == "vid" & .glid == "cs" & glid %in% isl.vid ~ glid,
             is.na(.loid) & .glid == "cs" & glid %in% cs.vessel ~ glid,
             is.na(.loid) & .glid == "cs2" ~ glid,
             .default = NA
           ),
         mmsi =
           case_when(
             is.na(.loid) & .glid == "mmsi" ~ glid,
             .default = NA))
keep2 |>
  mutate(defined = !is.na(vid) | !is.na(cs) | !is.na(mmsi)) |>
  group_by(defined, .loid, .glid) |> reframe(n = n(), pings = sum(pings)) |>
  arrange(desc(defined), -pings) |>
  mutate(p = round(pings / sum(pings) * 100, 2),
         pn = round(n / sum(n) * 100, 2),
         dp = cumsum(p),
         cn = cumsum(pn)) |>
  knitr::kable()
keep2 |>
  filter(.loid == "cs1", .glid == "cs") |>
  arrange(-pings) |>
  filter(loid %in% vessels$uid) |>
  knitr::kable()
...
drop01 <-
  stk |>
  filter(.glid %in% c("fixed", "hafro", "kvi", "navigational aid", "net", "unknown"))
drop02 <-
  stk |>
  filter(mmsi_cat %in% c("other"))
drop <-
  bind_rows(drop01,
            drop02)
keep <-
...
## Older below
vessels |>
  filter(str_starts(cs, "LH3"))

stk |>
  mutate(tmp = ifelse(nchar(glid) == 9, glid, NA)) |>
  mutate(cat = rb_mmsi_category(tmp)) |>
  count(cat)
...
keep <-
  ...
  select(-mmsi_cat)
# Test:
val_cap <- function(x, cap = 15) {
  ifelse(x > cap, cap, x)
}
keep |> group_by(.loid, .glid) |> reframe(n = n(), pings = sum(pings))



keep |> filter(.loid == "mmsi")


keep |> filter(.loid == "vid", is.na(.glid)) |> knitr::kable()
keep |>
  filter(is.na(.loid), .glid == "vid") |>
  arrange(-pings) |>
  mutate(has.mmsi = ifelse(glid %in% isl_vid_w.mmsi, TRUE, FALSE)) |>
  knitr::kable()

stk_trail(con) |>
  filter(mid == 122456) |>
  collect() |>
  mutate(speed = val_cap(speed, 15)) |>
  rb_mapdeck(radius = 10, add_harbour = FALSE)

keep |>
  mutate(vid =
           case_when(
             .loid == "vid" & .glid == "vid" ~ glid,
             .loid == "vid" & .glid == "cs" & str_starts(glid, "TF")  ~ loid,
             .loid == "vid" & .glid == "cs" & loid %in% as.character(3700:4999) ~ loid,
             .loid == "vid" & is.na(.glid) & str_starts(tolower(glid), "afma") ~ loid,
             # typo
             loid == "2500" & glid == "THAE" ~ loid,
             loid == "1807" & glid == "TMP_TFOP" ~ loid,
             loid == "2654" & glid == "TMP_TFCG" ~ loid,
             .default = NA)) |>
  mutate(has.vid = !is.na(vid)) |>
  group_by(has.vid, .loid, .glid) |> reframe(n = n(), pings = sum(pings))
...
keep |>
  ...
  stk |> group_by(.loid, .glid) |> reframe(n = n(), pings = sum(pings))

stk |>
  filter(!mid %in% drop$mid) |>
  group_by(.loid, .glid) |> reframe(n = n(), pings = sum(pings))
...
stk |>
  mutate(vid = case_when(
    .default = NA)) |>
  mutate(vid = as.integer(str_trim(vid))) |>
  filter(!is.na(vid)) |>
  arrange(-pings) |>
  knitr::kable()
...
stk |> group_by(.loid, .glid) |> reframe(n = n(), pings = sum(pings)) |> arrange(-pings)

stk2 <-
  stk |>
  mutate(vid = case_when(.loid == "vid" & .glid == "vid" ~ glid,
                         .default = NA)) |>
  mutate(vid = as.integer(vid))
stk |>
...
stk2 |>
  filter(is.na(vid)) |>
  group_by(.loid, .glid) |> reframe(n = n(), pings = sum(pings)) |> arrange(-pings)
stk2 |>
  filter(.loid == "vid", .glid == "cs") |>
  filter(!str_starts(glid, "TF")) |>
  arrange(loid) |>
  knitr::kable()
...
stk |>
  mutate(vid =
           case_when(
             .loid == "vid" & .glid == "vid" & loid == glid ~ loid,
             .loid == "vid" & .glid == "cs" & glid %in% isl_cs ~ loid,
             .loid == "vid" & .glid == "cs" & glid %in% oth_cs ~ loid,
             .default = NA)) |>
  filter(is.na(vid)) |>
  group_by(.loid, .glid) |> reframe(n = n(), pings = sum(pings)) |> arrange(-pings)
...
tmp <-
  ...
  mutate(
    ...
    keep =
      case_when(
        .loid == "vid" & .glid == "vid" & loid == glid & glid %in% isl_vid ~ "vid_vid",
        .loid == "vid" & .glid == "cs" &
          loid %in% isl_vid & glid %in% isl_cs ~ "vid_cs_isl",
        .default = NA))
...
vessels |>
  ...
  filter(cs == "TFTV")
mmsi <-
  nanoparquet::read_parquet("data/vessels/mmsi_ISL.parquet")
