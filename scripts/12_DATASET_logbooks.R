...
MOBILE_new <-
  ...
  select(station_id, sweeps, effort, effort_unit, in.old)
## Static gear -----------------------------------------------------------------
STATIC_new <-
  BASE_new |>
  inner_join(
  ...
  mutate(dt = as.numeric(difftime(t2, t1, unit = "hours")),
         ...
         effort_unit = case_when(gid == 3 ~ "hookhours",
                                 ...
                                 gid %in% 1 ~ "hooks",
                                 .default = NA)) |>
  select(station_id, effort, effort_unit, in.old)
## Dredge gear -----------------------------------------------------------------
DREDGE_new <-
  BASE_new |>
  inner_join(
  ...
  mutate(effort = as.numeric(difftime(t2, t1, units = "hours")),
         ...
         plow_width = 2) |>
  select(station_id, plow_width, effort, effort_unit, in.old)
## Trap gear -------------------------------------------------------------------
TRAP_new <-
  BASE_new |>
  inner_join(
...
LGS_new <-
  BASE_new |>
  left_join(BASE_new_aux |> select(-in.old),
            by = join_by(station_id)) |>
  select(.sid = station_id, vid, gid, date, t1, t2, lon, lat, lon2, lat2, z1, z2,
         datel, effort, effort_unit, sweeps, plow_width, base, in.old)

## Catch -----------------------------------------------------------------------
CATCH_new <-
  lb_catch_new(con) |>
  collect(n = Inf) |>
  inner_join(BASE_new |> select(station_id, in.old),
             by = join_by(station_id)) |>
