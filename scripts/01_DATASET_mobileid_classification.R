  ...
  mutate(
    .glid =
      case_when(
        ...
        glid %in% VID_mmsi_yes ~ "vid_isl_mmsi_yes",
        glid %in% VID_mmsi_no ~ "vid_isl_mmsi_no",
        # numbers_only(glid) & nchar(glid) == 9 ~ "mmsi",
        glid %in% CS_ISL_mmsi_yes ~ "cs_isl_mmsi_yes",
        glid %in% CS_ISL_mmsi_no  ~ "cs_isl_mmsi_no",
        glid %in% CS_OTH ~ "cs_oth",
        glid %in% UID_ISL_mmsi_yes ~ "uid_isl_mmsi_yes",
        glid %in% UID_ISL_mmsi_no ~  "uid_isl_mmsi_no",
...
stk2 <-
  ...
  mutate(.loid =
           case_when(
             ...
             # 
             loid %in% CS ~ "cs",
             loid %in% CS_ISL ~ "cs_isl",
             loid %in% CS_OTH ~ "cs_oth",
             loid %in% UID ~ "uid",
             loid %in% UID2 ~ "uid2",
