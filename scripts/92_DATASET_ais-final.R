...
#  nohup R < scripts/92_DATASET_ais-final.R --vanilla > scripts/log/04_stk-trails_parquet_2024-05-27.log &
#

# 2024-05-XX 
#  reduce the number of times doing st_as_sf- already coded
#  use ref to store what points are interpolated, drop the vms variable
...
harbour <- read_sf("data/auxillary/harbours.gpkg")
harbours.standards <- 
  readxl::read_excel("data-raw/harbours/harbours.xlsx") |> 
  select(hid, hid_std)
# This should be redone - more generic shapeid to use, then a lookup
eusm <- 
  read_sf("data/auxillary/EUSeaMap_2023.gpkg") |> 
  select(.euseamap_id)
#eusm <- 
#  eusm |> 
...
gebco_z <- read_sf("data/auxillary/ices_gebco.gpkg")
