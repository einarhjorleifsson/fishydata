...
This document reviews and illustrates the ICES VMS datacall workflow, highlighting both legacy and modern data processing approaches. The goal is to clarify the process, identify opportunities for improvement, and demonstrate how to transition from in-memory R workflows to scalable, database-backed analyses (via `dplyr` and `duckdb`). This is especially relevant as data volumes grow, and reproducibility and maintainability become ever more important.
...
tmp <- 
  trips |> 
  left_join(events) |> 
  rename(FT_DDATIM = D_DATIM,
         FT_LDATIM = L_DATIM) |> 
  filter(FT_REF == "272058") |> 
  select(-c(.tid, VE_LEN, VE_KW, VE_TON, weight, price))
ramb::dc_split_among_pings(
  trails |> select(-c(VE_LEN, VE_KW, VE_TON, weight, price)) |> filter(FT_REF == "272058"),
  tmp) |> 
  glimpse()
```
