...
format_vt_tacsat <- function(tacsat) {
  tacsat |> 
    dplyr::mutate(SI_DATE = stringr::str_replace(SI_DATE, "1800", "1803"),
                  SI_DATE = stringr::str_replace(SI_DATE, "1801", "1804")) |> 
    tidyr::unite(col = "SI_DDATIM", SI_DATE, SI_TIME, remove = FALSE) |> 
    dplyr::mutate(SI_DATIM = dmy_hms(SI_DDATIM)) |> 
    tibble::as_tibble()
}
format_vt_eflalo <- function(eflalo) {
  eflalo |> 
    ...
    tidyr::unite(col = "FT_DDATIM", FT_DDAT, FT_DTIME, remove = FALSE) |> 
    tidyr::unite(col = "FT_LDATIM", FT_LDAT, FT_LTIME, remove = FALSE) |> 
...
lb <- 
  ...
  mutate(.rid = 1:n(),
         .before = VE_REF) |> 
  # 1.3.3 Remove non-unique trip numbers - THIS IS A MISNOMER
  #       Effectively dropping duplicates of trip-gear-ir-catch-date
  #       Not sure if Kosher if the logbook records are individual settings
  #       where a vessel can have more than 1 setting per day within an ices rectangle
  distinct(LE_ID, LE_CDAT, .keep_all = TRUE) |> 
  # 1.3.4 Remove impossible time stamp records
  drop_na(FT_DDATIM, FT_LDATIM) |> 
  # 1.3.5 Remove trip starting before 1st Jan
  #       Interpreted here as trips that go over the new year
...
st <- 
  lb |> 
  select(.rid, VE_REF:FT_LDATIM, VE_FLT:LE_EFF_VMS)
```
...
tap <- 
  ...
  left_join(lb |> 
              dplyr::select(VE_REF,
                            ...
                            VE_LEN, 
                            VE_KW, 
                            LE_RECT, LE_MET, VE_FLT, VE_COU,
                            FT_DDATIM,
                            FT_LDATIM) |> 
              distinct(VE_REF, VE_COU, FT_REF, .keep_all = TRUE),
            by = join_by(VE_REF, VE_COU, between(SI_DATIM, FT_DDATIM, FT_LDATIM))) |> 
  dplyr::select(-c(FT_DDATIM, FT_LDATIM)) |> 
  dplyr::mutate(FT_REF = replace_na(FT_REF, "0")) |> 
  dplyr::filter(FT_REF != "0")
