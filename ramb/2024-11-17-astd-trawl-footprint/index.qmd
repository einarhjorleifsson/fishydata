...
* Some explanation of the variable names:
  ...
  * gid_ln_agf: gear id in the GAFL landings database
...
lgs <- 
  ...
  select(.sid, mmsi, vid, gid, gid_ln_agf, date, t1, t2, catch_total)
catch <- 
  open_dataset("/u3/haf/stasi/fishydata/data/logbooks/catch.parquet") |> 
  to_duckdb()
ais <- 
...
In the above we are getting quite a lot of false positives as can be observed by the implied fishing closer to the 12 miles and within the fjords.
...
So we are getting bit less of false positives close to shore as expected. But vessel captains are known to forget report at least the end of tows. So let's try a combo of both speed and time filter.
...
Not too bad as a first go! What is though really needed is some kind of an algorithm that treats the ais data as a time series where pre- and proceeding points are taken into account if a point is considered fishing or not.
