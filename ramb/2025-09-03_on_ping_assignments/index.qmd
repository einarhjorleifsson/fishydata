...
#### Think only of assigning LE_GEAR to vms pings

* If one gear per trip:
  * If LE_CDAT available assign that gear by trip and dates
* If more than one gear per trip and if/when LE_CDAT available 
  * Witin a LE_CDAT assign the gear with the highest catches to the ping


* Assume that FT_REF is mandatory in the logbook data, however LE_CDAT may or may not be reported
  * If LE_CDAT available:
    * Within a date assign the gear with the highest catch to the pings

* Get trips with more than one LE_GEAR
  * Match lb and vms by FT_REF and SI_DATE-LE_CDAT
...
trip_assign <- function(tacsatp, eflalo, col = "LE_GEAR", haul_logbook = F) {
  ...
  # 1) Get the trips (LE_REF) with more than one gear ( or other columns ) reported
...
Should one not, in this very specific case, do something like this:

```{r}
lb |> 
  group_by(FT_REF, LE_CDAT) |> 
  mutate(max_catch = max(LE_KG_TOT),
         max_gear = LE_GEAR[LE_KG_TOT == max_catch]) |> 
  group_by(FT_REF, LE_CDAT, LE_GEAR = max_gear) |> 
  reframe(LE_KG_TOT = sum(LE_KG_TOT))
```
