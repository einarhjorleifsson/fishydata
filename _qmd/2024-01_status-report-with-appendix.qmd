...
fig-height: 6
crossref:
  fig-prefix: mynd   # (default is "Figure")
  tbl-prefix: tafla    # (default is "Table")
---
...
TRIP_VISIR <- 
  ...
  filter(!is.na(.sid))
lb <- 
  arrow::open_dataset(here::here(paste0(pth, "logbooks/parquet/station-for-ais.parquet"))) |> 
...
Ef úrtakið á eftirlitsferðum er tilviljunarkennt þá eru þetta sterkar vísbendingar um tegundarháð brottkast. Saman mynstur kemur þó upp fyrir botnvörpu og línu ef greiningin er bundin einungis við skip þar sem eftirlitsmaður hefur verið um borð i a.m.k. einu tilfelli (niðurstöður ekki sýndar).
...
## Útbreiðsla veiða eftir tegundum
...
Samanburður á tegundarsamsetningu í lönduðum afla eftir því hvort eftirlitsmaður er um borð eður ei bendir til að tegundabrottkast geti verið umtalsvert. Þar sem veiðar á hverri tegund er háð staðsetningu þá var ætlunin að nýta slíkar upplýsingar úr afladagbókum sem og úr stk gögnum til að greina betur meint tegundarbrottkast. Greining á afladagbókum bendir þó til að skráning á aflasamsetningu í hverju togi þegar kemur að tegundum er mjög svo ónákvæm. Á þessu stigi er ekki ljóst hvaða áhrif sú bjögun myndi hafa á slíka greiningu né hvort að hægt sé með einhverjum hætti að taka tillit til hennar með einhverjum hætti.

## Viðbætur


### Vessel possition data

* Live feed data, every 10 minutes
* Ekki einkvæmur lykill við skipaskrárnúmer. ...
* Signal ekki þegar gögn eru í höfn
* Virðist vera mikið villum sem koma fram þegar skip er skráð að koma inn eða úr höfn
* Hafnir skilgreindar sem flákar.

Vessel position data from 2009 onwards were obtained from the Icelandic Coast Guard. The archieved data were less freque.

Gögn fyrir staðsetningu skipa koma frá Landhelgisgæslunni (Sjálvirk tilkynningarskilda skip, STK).

Tenging við skipaskrárnúmer eru að mestu fengin með því að

## Landings records


```{r}
#| label: tbl-landspecies1
#| tbl-cap: "Fjöldi mælinga (n), meðalfjöldi tegunda landað, mismunir í fjölda og staðalskekkja."
ln |> 
  group_by(gear, Eftirlitsmaður) |> 
  summarise(n = n(),
            Tegundir = mean(n_sid),
            Staðalfrávik = sd(n_sid),
            Staðalskekkja = Staðalfrávik / sqrt(n),
            .groups = "drop") |> 
  group_by(gear) |> 
  mutate(mismunur = Tegundir - lead(Tegundir)) |> 
  ungroup() |> 
  select(Veiðdarfæri = gear, Eftirlitsmaður:Tegundir, mismunur, Staðalskekkja) |> 
  knitr::kable(digits = 2, escape = )
```

or this way ...

```{r}
#| label: tbl-landspecies2
ln |> 
  group_by(gear, Eftirlitsmaður) |> 
  summarise(Tegundir = mean(n_sid),
            .groups = "drop") |> 
  spread(Eftirlitsmaður, Tegundir) |> 
  mutate(#Mismunur = Já - Nei,
         Hlutfall = Já / Nei) |> 
  rename(Veiðarfæri = gear) |> 
  arrange(desc(Hlutfall)) |> 
  knitr::kable(caption = "Meðalfjöldi tegunda landað eftir veiðarfærum eftir því hvort eftirlitsmaður er um borð (Já) eða ekki (Nei) og hlutfallslegur fjöldi sem er landað þegar eftirlitsmaður er um borð.", digits = 2, escape = )
```


## Logbooks

The logbook records from 2009 onwards are stored in two different databases where the structure are to some degree different.

* Frá árinu 20xx var tekið upp nýtt kerfi
* Eldra og yngra kerfi

The structure of the logbook database is somewhat complicated in the sense that effort data for each gear are stored in different xxx
